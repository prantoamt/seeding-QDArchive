...
@pytest.fixture
def sample_project():
    """Insert a sample project with files, keywords, and persons into the DB."""
    session = get_session()
    project = Project(
        query_string="qualitative",
        repository_id=4,
        repository_url="https://data.qdr.syr.edu",
        project_url="https://data.qdr.syr.edu/dataset.xhtml?persistentId=doi:10.5064/F6ABC123",
        title="Test Dataset",
        description="qualitative interview transcripts",
        language="en",
        doi="https://doi.org/10.5064/F6ABC123",
        upload_date="2024-01-01",
        download_repository_folder="qdr",
        download_project_folder="10.5064_F6ABC123",
        download_method=DownloadMethod.API_CALL,
        license_type="CC-BY-4.0",
        restricted=True,
        software="NVivo 12",
        depositor="Doe, A.",
        producer="University of Testing",
    ...
    project.keywords.append(Keyword(keyword="qualitative research"))
    project.keywords.append(Keyword(keyword="interviews"))
    project.persons.append(PersonRole(name="Smith, J.", role=PersonRoleType.AUTHOR))
    session.add(project)
    session.flush()

    session.add(ProjectFile(
        project_id=project.id,
        file_name="analysis.qdpx",
        file_type=".qdpx",
        is_qda_file=True,
    ...
    session.add(ProjectFile(
        ...
        local_path="data/qdr/10.5064_F6ABC123/transcript.pdf",
        file_hash="abc123",
...
def test_status_with_records(runner, sample_project):
    result = runner.invoke(cli, ["status"])
    assert result.exit_code == 0
...
def test_db_with_records(runner, sample_project):
    result = runner.invoke(cli, ["db"])
    assert result.exit_code == 0
    assert "Test Dataset" in result.output


def test_db_qda_only(runner, sample_project):
    result = runner.invoke(cli, ["db", "--qda-only"])
    assert result.exit_code == 0
    assert "Test Dataset" in result.output


def test_db_restricted_only(runner, sample_project):
    result = runner.invoke(cli, ["db", "--restricted-only"])
    assert result.exit_code == 0
    assert "Test Dataset" in result.output


def test_show_project(runner, sample_project):
...
def test_show_project_extra_fields(runner, sample_project):
    """Verify show command displays extra metadata."""
    result = runner.invoke(cli, ["show", "1"])
    assert result.exit_code == 0
    assert "Doe, A." in result.output
    assert "University of Testing" in result.output
    assert "Smith (2023)" in result.output
...
def test_export(runner, sample_project, tmp_path):
...
def test_reset_confirmed(runner, sample_project):
...
def test_search_with_connector(runner):
    mock_results = [
        MagicMock(
            title="Test Dataset",
            persons=[{"name": "Doe, A.", "role": "AUTHOR"}],
...
def test_db_search(runner, sample_project):
    result = runner.invoke(cli, ["db", "--search", "interview"])
    assert result.exit_code == 0
    assert "Test Dataset" in result.output


def test_db_language(runner, sample_project):
    result = runner.invoke(cli, ["db", "--language", "en"])
    assert result.exit_code == 0
    assert "Test Dataset" in result.output
